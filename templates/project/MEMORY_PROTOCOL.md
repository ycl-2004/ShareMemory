...
- **AUTO-WRITE** (immediately, unasked): architectural/technical decisions and dependency changes → `DECISIONS.md`; goal/structure changes → `PROJECT.md`; on completing a task → update today's `SYNC_LOG.md` block (full `TASKS.md` update stays manual).
- **MANUAL-WRITE** (only when user invokes the share-memory skill / says "update memory"): `TASKS.md` progress, `LEARNINGS.md`.
...
User instruction wins — BUT explicitly point out any conflict with memory and confirm before proceeding; then update memory. Never silently override either side.

## 8. Size Control & Maintenance
